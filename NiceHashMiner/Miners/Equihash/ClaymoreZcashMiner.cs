...
namespace NiceHashMiner.Miners {
    public class ClaymoreZcashMiner : ClaymoreBaseMiner {
        ...
        public ClaymoreZcashMiner()
            : base("ClaymoreZcashMiner", MinerPaths.ClaymoreZcashMiner, "ZecMiner64", _LOOK_FOR_START) {
        }

        
        public override void Start(string url, string btcAdress, string worker) {
            string username = GetUsername(btcAdress, worker);
            LastCommandLine = " " + GetDevicesCommandString() + " -mport -" + APIPort + " -zpool " + url + " -zwal " + username + " -zpsw x";
